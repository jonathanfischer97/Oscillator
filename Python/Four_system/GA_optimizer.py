...
class Oscillator: 
	...
	def __init__(self, parameter_values, params, initial_conditions, dt = 0.01, mode = 0): 
	...
	def plotModel(self, subject, mode="ode", show=True):     		
		...
		# lines = plt.plot(ts, p0, ts, p1, ts, p2, ts, p3)  
		...
		plt.plot(t,solutions[:,0], color="b", label = 'Xn')
		plt.plot(t,solutions[:,1], color="g", label = 'Xc')
		plt.plot(t,solutions[:,2], color="gold", label = 'Yc')
		plt.plot(t,solutions[:,3], color="r", label = 'Yn')
...
parameter_values = {  "sig": {"min": 100, "max": 1500},  
            "hill": {"min": 1, "max": 5}, 
            "kdxn": {"min": 0.1, "max": 50},            				
            "kexport": {"min": 0.1, "max": 50},        
            "kdxc": {"min": 0.1, "max": 50},         
            "eps": {"min": 0.1, "max": 10},         
            "kdyn": {"min": 0.1, "max": 50}, 
            "kdyc": {"min": 0.001, "max":10}, 
            "Km": {"min": 0.001, "max":10},
            "kimport": {"min": 0.001, "max":10}     	
            }    	 


filename =  os.path.join(".", "oscillatorCostTwo")  
print(filename)   
model = Oscillator(parameter_values, np.array(["sig", "hill", "kdxn", "kexport", "kdxc", "eps", "kdyn", "kdyc", "Km", "kimport"]), np.array([0, 40, 100, 50]), mode=1) 
solver = Solver(model)         
solver.run(filename) 
...
folder1 = os.path.join(".", "oscillatorCostTwo_Region0ViableSet_Iter10.p")      				
...
model2 = Oscillator(parameter_values, np.array(["sig", "hill", "kdxn", "kexport", "kdxc", "eps", "kdyn", "kdyc", "Km", "kimport"]), np.array([0, 40, 100, 50]), mode=1) 
...
for i in range(numSamples + 3): 
	...
	lines = plt.plot(t, Y_ode[:,0], t, Y_ode[:,1], t, Y_ode[:,2], t, Y_ode[:,3])   
